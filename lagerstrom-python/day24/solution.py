...
def main():
    ...
    all_components = parse_input('small_input.txt')
    ...
    for bridge in all_bridges:
        #print('')
        #print(bridge.get_last_component())
        for component in all_components[bridge.get_next_port()]:
            if bridge.component_in_bridge(component) == False:
                new_bridge = Bridge()
    ...
    highest_score = 0
    for bridge in all_bridges:
        if bridge.calculate_score() > highest_score:
            highest_score = bridge.calculate_score()
